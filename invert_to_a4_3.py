...
def process(input_path: str, output_prefix: str, scale: float = 2.0) -> None:
    img      = Image.open(input_path).convert("RGB")
    inverted = binarize_and_invert(img)

    src_w, src_h = inverted.size
    # 寬度永遠等比縮放至 A4 內容區寬度（避免水平溢出）
    # 高度依相同比例放大後再乘 scale 倍 → 超出部分由分頁處理
    fit_scale = CONTENT_W / src_w        # 基準：寬度剛好填滿 A4
    new_w     = CONTENT_W
    new_h     = round(src_h * fit_scale * scale)  # 高度放大 scale 倍
    ...
    for f in saved:
        print(f"   {f}")
    print(f"   內容尺寸：{new_w} x {new_h} px（放大 {scale}x）")
